...
def extract_orders(session: Session) -> pd.DataFrame:
    ...
    logger.info("Extracting raw orders from ORDERS_STAGING_V2 ...")
    df = session.sql("SELECT * FROM ORDERS_STAGING_V2").to_pandas()
    logger.info("Extracted %d rows from ORDERS_STAGING_V2.", len(df))
    return df
